        ...
        <Draggable draggableId={header.id} key={header.id} index={index} isDragDisabled={!!isPinned}>
            {(provided, snapshot) => (
                <th key={header.id} colSpan={header.colSpan} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}
                    ...
                    className={`border-r border-gray-500 px-3 py-2 text-nowrap ${snapshot.isDragging ? "bg-slate-500" : ""} ${isPinned ? "bg-slate-600 text-white" : ""}`}>

                    {isOpen && ReactDOM.createPortal(
                        <div className="dropdown-menu absolute z-50 bg-[#353434] text-white border border-gray-500 rounded shadow-md"
                            style={{ top: menuPosition.top, left: menuPosition.left }}>
                            <ul className="text-sm" onClick={() => setIsOpen(false)}>
                                {isPinned !== "right" && (
                                    <li onClick={() => { header.column.pin("right"); setIsOpen(false); }} className="px-4 py-2 hover:bg-gray-400 cursor-pointer">
                                        Pin to Right
                                    </li>
                                )}
                                {isPinned !== "left" && (
                                    <li onClick={() => { header.column.pin("left"); setIsOpen(false); }} className="px-4 py-2 hover:bg-gray-400 cursor-pointer">
                                        Pin to Left
                                    </li>
                                )}
                                {isPinned && (
                                    <li onClick={() => { header.column.pin(false); setIsOpen(false); }} className="px-4 py-2 hover:bg-gray-400 cursor-pointer">
                                        Unpin
                                    </li>
                                )}
                                <li onClick={header.column.getToggleSortingHandler()} className="px-4 py-2 hover:bg-gray-400 cursor-pointer">
