        ...
        <div className="bg-[#272727] text-white border-2 border-gray-500 h-fit w-fit rounded-xl">
          <table className="min-w-[1500px] rounded-lg overflow-hidden">
            ...
            <tbody className="rounded-xl overflow-x-auto bg-[#272727]">
              {table.getRowModel().rows.map((blog) => (

                <Fragment key={blog.id}>
                  <tr className={`overflow-x-scroll h-fit ${blog.getIsSelected() && "border-4 border-blue-500"}`} >
                    {blog.getVisibleCells().map((cell) => (
                  ...
                  {blog.getIsExpanded() && (
                    <tr>
                      <td colSpan={blog.getVisibleCells().length}>
                        <RowDetailView blog={blog.original} />
                      </td>
                    </tr>
                  )}
                </Fragment>
